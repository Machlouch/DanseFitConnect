...
export class CourseService {
  ...
  private baseFRONT = "http://localhost:8081/annulation";  
  ...
  supprimerCours(courseID: number): Observable<any> {
    const url = `http://localhost:8081/course/delete/${courseID}`;
    return this.http.delete(url);
  }
  

  changerEtat(courseID: number): Observable<any> {
    const url = `http://localhost:8081/course/update-annulation/${courseID}`;
    return this.http.put(url, {});
  }
  

  refus(courseID: number): Observable<any> {
    const url = `http://localhost:8081/course/refus-annulation/${courseID}`;
    return this.http.put(url, {});
  }
